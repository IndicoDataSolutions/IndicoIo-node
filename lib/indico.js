...
var api_request = function (api, batch) {
  return function (data, config) {
    ...
    if (api.type === "training" && !settings.resolvePrivateCloud(config, indico.cloud)) {
      throw("Training APIs are only available to private cloud users. If you are on a private cloud, make sure you have set the private cloud configuration");
    }
...
services.models.forEach(function (api) {
...
indico["training"] = {}
services.training.forEach(function(service) {
  indico.training[service.name] = api_request(service, false);
});
